# Pebble Mind editorial calendar

| Date | Asset | Audience | Channels | Status |
|---|---|---|---|---|
| 2026-09-03 | Private AI on iPhone: a simpler way to try it | Everyday privacy-conscious users | Website blog, LinkedIn, Reddit, X, Shorts/Reels/TikTok | Drafted; human publishing required |
| 2026-09-04 | Getting started with Pebble Mind in three minutes | New iPhone/iPad AI users | Website blog, LinkedIn, Reddit, X, Shorts/Reels/TikTok | Ready on site; adaptations queued |
| 2026-09-05 | Five everyday things to try with private AI | Everyday privacy-conscious users | Website blog, LinkedIn, Reddit, X, Shorts/Reels/TikTok | Source drafted; adaptations ready |
| 2026-09-06 | Localized “try private AI” posts | Everyday users worldwide | X, Instagram, TikTok, Shorts, Bluesky, Mastodon, local communities | Drafted; fluent review required |
| 2026-09-07 | Does AI need to see everything? | Everyday privacy-conscious users | Website blog, LinkedIn, Reddit, X, Shorts/Reels/TikTok | Ready on site; adaptations queued |
| 2026-09-09 | First-version local AI, explained clearly | Developers, privacy users, and local-AI communities | Hacker News, Bluesky, Mastodon, individualized outreach | Drafted; authenticated publishing and prospect research required |

Start the next cycle from `growth/START_HERE.md`.
| Next cycle | Three everyday uses for local AI | Apple users and families | Website blog, social, short video | Queued |
| Next cycle | How to start with Pebble Mind | New AI users | Website blog, Reddit, X, video | Queued |
| Next cycle | Why local AI matters for personal notes | Privacy-conscious users | Website blog, LinkedIn, Reddit, X | Queued |
| Next cycle | What we are improving in version one | Existing users | Website blog, social, Product Hunt update | Queued |

Cadence: one blog post and its adaptations per cycle; three Reddit replies; five X posts; two LinkedIn posts; three short videos; three Bluesky/Mastodon posts; and five individualized outreach notes. Never publish identical copy repeatedly across communities.
